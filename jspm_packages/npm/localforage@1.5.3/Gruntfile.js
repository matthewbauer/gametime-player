  ...
  module.exports = exports = function(grunt) {
    ...
    grunt.initConfig({
      ...
      browserify: {
        ...
        main: {
          files: {'dist/localforage.js': 'src/localforage.js'},
          options: {
            browserifyOptions: {standalone: 'localforage'},
            transform: ['rollupify', 'babelify'],
            plugin: ['bundle-collapser/plugin', 'browserify-derequire']
          }
        },
        no_promises: {
          files: {'dist/localforage.nopromises.js': 'src/localforage.js'},
          options: {
            browserifyOptions: {standalone: 'localforage'},
            transform: ['rollupify', 'babelify'],
            plugin: ['bundle-collapser/plugin', 'browserify-derequire'],
      ...
      eslint: {target: sourceFiles},
      mocha: {unit: {options: {urls: ['http://localhost:9999/test/test.main.html', 'http://localhost:9999/test/test.min.html', 'http://localhost:9999/test/test.callwhenready.html', 'http://localhost:9999/test/test.customdriver.html', 'http://localhost:9999/test/test.faultydriver.html', 'http://localhost:9999/test/test.nodriver.html', 'http://localhost:9999/test/test.browserify.html', 'http://localhost:9999/test/test.require.html', 'http://localhost:9999/test/test.webpack.html']}}},
      ...
      watch: {
        ...
        'mocha:unit': {
          ...
          tasks: ['eslint', 'browserify:package_bundling_test', 'webpack:package_bundling_test', 'mocha:unit']
        }
      },
    ...
    grunt.registerTask('build', ['browserify:main', 'browserify:no_promises', 'concat', 'es3_safe_recast', 'uglify']);
    grunt.registerTask('serve', ['build', 'connect:test', 'watch']);
    var testTasks = ['build', 'babel', 'eslint', 'ts:typing_tests', 'browserify:package_bundling_test', 'webpack:package_bundling_test', 'connect:test', 'mocha'];
